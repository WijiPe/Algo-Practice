 ...
 * - Space: O(1) constant.
 */
function binarySearch(sortedNums, searchNum, leftIdx, rightIdx) {
...
function exponentialSearch(sortedNums, searchNum) {
  ...
  return binarySearch(
    ...
    searchNum,
    i / 2,
    Math.min(i, sortedNums.length-1)
  );
}

let idx = exponentialSearch(nums4,2)
console.log(countAdjacentDupes(nums4, idx));